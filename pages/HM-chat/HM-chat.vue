...
<script>
	import { websocetObj } from '../../static/js/websocket';
	export default {
		data() {
			return {
				//文字消息
		...
		onLoad(option) {
			...
			//取记录
			try {
			    const value = uni.getStorageSync('robot_chat');
				console.log(value)
			    if (value) {
			        this.msgList = value
			    }
			...
			websocetObj.sokcet(this.getWebsocetData,this.getWebsocetError,this.onErrorSucceed)
		...
		methods:{
			...
			// val = String.fromCharCode.apply(null, new Uint8Array(val)).trim()  如果后端返回数据格式是其他的，可能需要转换一下，比如这个，应该是转Unicode编码
				...
				let msg = {
					...
					msg:{
						...
						userinfo:{
							uid:1,
							username:"陪聊机器人",
							face:"/static/img/im/face/robot.png"},
							content:{
								text: message.msg.content.text,
							},
						},
					}
				// 本地模拟发送消息
				...
				// 存储本地
				//this.msgList.push(msg)
				uni.setStorageSync("robot_chat", this.msgList)
